#! /usr/bin/env python3
# orginal author https://github.com/mamins1376/Subscene-API
#
# -*- coding: utf-8 -*-
...
def search(term, language="", limit_to=SearchTypes.Exact):
    ...
    for _ in range(10):
        r = request_session.post(SITE_DOMAIN + '/subtitles/searchbytitle',
                                 data={'query':term, 'l':language, 'g-recaptcha-response':'0'},
                                 headers=HEADERS)
...
def sleep():
    ...
    print("Error searching, sleeping for %f sec"%(delay))
    time.sleep(delay)
    lastcall=time.time()